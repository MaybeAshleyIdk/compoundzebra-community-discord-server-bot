...
public class ImplProjectPlugin : Plugin<Project> {
	...
	private fun checkPlugins(project: Project) {
		...
		project.plugins.withType<PartitionedProjectPlugin> {
			error("The implementation $project has the partitioned project plugin applied to it, but it must not")
		}
		...
		project.plugins.withType<WiringAssimilationProjectPlugin> {
			val msg: String = "The implementation $project has the wiring-assimilation project plugin applied to it" +
				", but it must not"
			error(msg)
		}
	}
